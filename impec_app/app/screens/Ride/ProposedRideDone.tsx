...
// Propriétés du composant ProposedRideDone
interface ProposedRideDoneProps {
  isDone?: Boolean;
  index: Number;
}

const ProposedRideDone: React.FC<ProposedRideDoneProps> = ({isDone, index}) => {
  const {API} = useApi();
  ...
  const readAgendaCourse = async () => {
    try {
      const result = await API.getCourseAgenda({
        ...
        subtype_course: EStatusCourse.TERMINE,
  ...
  const userQuery = useQuery('proposed_done', readAgendaCourse);
  const {
    data: dataAgenda,
      ...
      <View style={styles.item}>
        {item?.etat_course === 'TERMINATED' && (
          <View
            ...
            <Text style={{color: '#ffffff'}}>Course fait par </Text>
        ...
        <TouchableOpacity
          onPress={() => navigation.navigate('DetailsRide', {data: item})}
          style={{
            flexDirection: 'row',
            alignItems: 'center',
          }}>
...
export default ProposedRideDone;
